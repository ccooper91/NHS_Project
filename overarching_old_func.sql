if object_ID('stage.overacrching_old_func') is not null
	begin
		drop function stage.overacrching_old_func
	end
go

create function stage.overacrching_old_func(@admiage int, @epistart varchar(20), @epiend varchar(20),
	@admimeth varchar(10), @Hcode varchar(10))

/*************************************************************************************************
Function to find the old tariff, and tariff of a medical procedure from the NHS price 
tariff documents.
...
select stage.overacrching_old_func(@admiage, @epistart, @epiend, @admimeth, @Hcode)
...
select @old = stage.old_tariff_calc(@elec, @staydays, @reducshort, @Hcode)
