if object_ID('stage.old_tariff_calc') is not null
	begin
		drop function stage.old_tariff_calc
	end
go
create function stage.old_tariff_calc(@elec int, @staydays int, @reducshort int, @Hcode varchar(10))
...
from stage.admitted1718
)

/* Add values from all columns from CTE above to get final tariff cost */
select 
	@old_tariff = isnull(DayTariff,0) + isnull(LongStayCharge,0) + isnull(ReductionTariff,0)
	from cte
